...
//dd($value->iemrepairOrder);
                $labour_details[$key]['id'] = $value->id;
                $labour_details[$key]['labour_id'] = $value->rot_iem_id;
               $labour_details[$key]['code'] = $value->iemrepairOrder->rot_code;
                $labour_details[$key]['name'] = $value->iemrepairOrder->name;
                $labour_details[$key]['km'] = $value->iemrepairOrder->km;
                $labour_details[$key]['job_group'] = $value->iemrepairOrder;
               // $labour_details[$key]['type'] = $value->iemrepairOrder->repairOrderType ? $value->repairOrder->repairOrderType->short_name : '-';
                $labour_details[$key]['qty'] = $value->qty;
                $iem_repair_order = $value->iemrepairOrder;
                if ($value->iemrepairOrder->is_editable == 1) {
                    $labour_details[$key]['rate'] = $value->amount;
                    $repair_order->amount = $value->amount;
                } else {
                    $labour_details[$key]['rate'] = $value->iemrepairOrder->onsite_price;
                }
                ...
                $labour_details[$key]['rot_item_order'] = $iem_repair_order;
                ...
                $labours[$key]['code'] = $value->iemrepairOrder->rot_code;
                $labours[$key]['name'] = $value->iemrepairOrder->name;
            }
        }
    ...
    public function saveLabourDetail(Request $request)
    {
        ...
        try {
            ...
            $repair_order = RepairOrder::find($request->rot_iem_id);
            if (!$repair_order) {
                return response()->json([
                    'success' => false,
                    'error' => 'Validation Error',
           ...
           // $on_site_repair_order->repair_order_id = $request->rot_id;
            $on_site_repair_order->repair_order_id = $request->rot_iem_id;
    ...
    public function saveRotLabourDetail(Request $request)
    {
         //dd($request->all());
        try {
            $error_messages = [
                'rot_iem_id.unique' => 'Labour is already taken',
            ];
            ...
            $validator = Validator::make($request->all(), [
                ...
                'rot_iem_id' => [
                        'required',
                        'integer',
                        'exists:rot_iem_details,id',
                        'unique:on_site_order_repair_orders,rot_iem_id,' . $request->rot_iem_id . ',id,on_site_order_id,' . $request->on_site_order_id,
                     ],
                // 'rot_id' => [
                //     'required',
                //     'integer',
            ...
            //$repair_order = RepairOrder::find($request->rot_iem_id);
            $repair_order = RotIemDetail::find($request->rot_iem_id);
            if (!$repair_order) {
                return response()->json([
                    'success' => false,
                    'error' => 'Validation Error',
            ...
            // if (!empty($request->on_site_repair_order_id)) {
            //     $on_site_repair_order = OnSiteOrderRepairOrder::find($request->on_site_repair_order_id);
            if (!empty($request->rot_iem_id)) {
                $on_site_repair_order = OnSiteOrderRepairOrder::find($request->rot_iem_id);
                if ($on_site_repair_order) {
                    $on_site_repair_order->updated_by_id = Auth::user()->id;
                    $on_site_repair_order->updated_at = Carbon::now();
                    $on_site_repair_order->removal_reason_id = null;
            ...
            $on_site_repair_order->on_site_order_id = $request->on_site_order_id;
           // $on_site_repair_order->repair_order_id = $request->rot_id;
            $on_site_repair_order->rot_iem_id = $request->rot_iem_id;
            //$on_site_repair_order->qty = $repair_order->hours;
            $on_site_repair_order->qty = $repair_order->total_working_hrs;
            $on_site_repair_order->split_order_type_id = $request->split_order_type_id;
            $on_site_repair_order->estimate_order_id = $estimate_order_id;
            // if ($request->repair_order_description) {
            //$on_site_repair_order->amount = isset($request->repair_order_amount) ? $request->repair_order_amount : $repair_order->amount;
            $on_site_repair_order->amount = isset($request->repair_order_amount) ? $request->repair_order_amount : $repair_order->onsite_price;
            // } else {
            // $on_site_repair_order->amount = $repair_order->amount;
            // }
