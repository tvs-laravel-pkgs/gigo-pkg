...
class VehicleInspectionItemGroupController extends Controller {
	...
	public function getVehicleInspectionItemGroupList(Request $request) {
		...
		return Datatables::of($vehicle_inspection_item_groups)
			...
			->addColumn('name', function ($vehicle_inspection_item_group) {
				$status = $vehicle_inspection_item_group->status == 'Active' ? 'green' : 'red';
				return '<span class="status-indicator ' . $status . '"></span>' . $vehicle_inspection_item_group->name;
			})
			->addColumn('action', function ($vehicle_inspection_item_group) {
				...
				$output = '';
				if (Entrust::can('edit-vehicle_inspection_item_group')) {
					$output .= '<a href="#!/gigo-pkg/vehicle_inspection_item_group/edit/' . $vehicle_inspection_item_group->id . '" id = "" title="Edit"><img src="' . $img1 . '" alt="Edit" class="img-responsive" onmouseover=this.src="' . $img1 . '" onmouseout=this.src="' . $img1 . '"></a>';
				}
				if (Entrust::can('delete-vehicle_inspection_item_group')) {
					$output .= '<a href="javascript:;" data-toggle="modal" data-target="#vehicle_inspection_item_group-delete-modal" onclick="angular.element(this).scope().deleteVehicleInspectionItemGroup(' . $vehicle_inspection_item_group->id . ')" title="Delete"><img src="' . $img_delete . '" alt="Delete" class="img-responsive delete" onmouseover=this.src="' . $img_delete . '" onmouseout=this.src="' . $img_delete . '"></a>';
				}
				return $output;
			})
			->make(true);
