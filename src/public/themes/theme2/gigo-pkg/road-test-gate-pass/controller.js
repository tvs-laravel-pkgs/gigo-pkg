app.component('roadTestGatePassCardList', {
    templateUrl: road_test_gate_pass_card_list_template_url,
    controller: function($http, $location, HelperService, $scope, $rootScope, $route) {
        ...
        if (!self.hasPermission('road-test-gate-passes')) {
        ...
        $scope.fetchData = function() {
            $.ajax({
                    url: base_url + '/api/road-test-gate-pass/get',
                    method: "POST",
                    data: {
        ...
        $scope.listRedirect = function(type) {
            if (type == 'table') {
                ...
                window.location = "#!/road-test-gate-pass/card-list";
                return false;
            }
        }
...
app.component('roadTestGatePassTableList', {
    templateUrl: road_test_gate_pass_list_template_url,
    controller: function($http, $location, HelperService, $scope, $rootScope, $route) {
        $rootScope.loading = true;
        var self = this;
        HelperService.isLoggedIn()
        ...
        $scope.clear_search = function() {
            self.search_key = '';
            $('#road_test_gate_pass_list').DataTable().search('').draw();
        }
        $('.refresh_table').on("click", function() {
            $('#road_test_gate_pass_list').DataTable().ajax.reload();
        });

        var dataTables = $('#road_test_gate_pass_list').dataTable();
        $scope.searchKey = function() {
        ...
        // FOR FILTER
        $http.get(
            laravel_routes['getRoadTestGatePassFilter']
        ).then(function(response) {
            self.extras = response.data.extras;
        });
...
app.component('roadTestGatePassView', {
    templateUrl: road_test_gate_pass_view_template_url,
    controller: function($http, $location, HelperService, $scope, $rootScope, $route, $routeParams) {
        $rootScope.loading = true;
        var self = this;
        HelperService.isLoggedIn()
        self.hasPermission = HelperService.hasPermission;
        if (!self.hasPermission('view-road-test-gate-pass')) {
            window.location = "#!/page-permission-denied";
            return false;
        }
        ...
        $scope.fetchData = function() {
            //VIEW GATE PASS
            $.ajax({
                url: base_url + '/api/road-test-gate-pass/view/get-data',
                type: "POST",
                data: {
        ...
        $scope.saveGatePass = function(id) {
            ...
            var v = jQuery(form_id).validate({
                ...
                submitHandler: function(form) {
                    ...
                    $.ajax({
                            url: base_url + '/api/road-test-gate-pass/gate-in-out/save',
                            method: "POST",
                            data: formData,
                            processData: false,
                            contentType: false,
