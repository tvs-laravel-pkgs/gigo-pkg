			...
			$i = 0;
                foreach ($rows as $k => $row) {
                    ...
                    if (empty($record['Dealer Order Number'])) {
                        ...
                        $job_card_billed_details->dealer_order_number=$record['Dealer Order Number'];
                    }
                    ...
                    if (empty($record['Customer Service Contact'])) {
                        ...
                        $job_card_billed_details->customer_mobile=$record['Customer Service Contact'];
                    }
    ...
    public static function importFromExcelJCBilled($job){
        try {
			...
			foreach ($rows as $k => $row) {
                ...
                DB::beginTransaction();
                $job_detail_report= new JobDetailReport();
                if (empty($record['Company Code'])) {
                    $status['errors'][] = 'Company Code is empty';
                } else {
                ...
                if (empty($record['KCC %'])) {
                    $status['errors'][] = 'KCC % is empty';
                     ...
                     $job_detail_report->kkc_percentage = $record['KCC %'];
                }
                ...
                if (empty($record['TCS'])) {
                    $status['errors'][] = 'TCS is empty';
                     ...
                     $job_detail_report->tvs_percentage = $record['TCS'];
                }
